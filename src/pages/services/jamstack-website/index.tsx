...
export interface JamstackQuery {
  technologies: ContentfulData<SectionModel<TechContentType>>;
}

export default function JamstackWebsitePage({ location }: PageProps) {
  const { technologies }: JamstackQuery = useStaticQuery(graphql`
    query JamstackPage {
  ...
  return (
    <DefaultLayout heroCtaRef={ctaRef}>
      ...
      <SplitSection
        ...
        title={
          <>
            Search engines <mark>love fast websites</mark> and people love it
            too.
          </>
        }
